...
import { useAssessmentAttempt, useAssessments } from '@/hooks';
import type { AssessmentQuestion, AssessmentQuestionSet, SubmitAssessmentAnswerRequest } from '@/types/assessment';
...
function readSavedAnswers(attemptId: string) {
  const storageKey = `sagelms.AssessmentAttempt.${attemptId}.answers`;
...
export default function QuestionTakingPage() {
  ...
  const { fetchQuestionSet } = useAssessments();
  const { startQuestionSetAttempt, submitAttempt, loading } = useAssessmentAttempt();
  ...
  const draftStorageKey = attemptId ? `sagelms.AssessmentAttempt.${attemptId}.answers` : '';
  ...
  const submit = useCallback(async (options?: { timeExpired?: boolean; confirmBeforeSubmit?: boolean }) => {
    ...
    const payload: SubmitAssessmentAnswerRequest[] = questions.map((question) => {
      const answer = currentAnswers[question.id] || {};
      return {
  ...
  }, [attemptId, confirm, courseId, draftStorageKey, id, navigate, questions, showToast, submitAttempt]);
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="space-y-4">
        {questions.map((question, index) => (
          <div key={question.id} id={`Assessment-question-${question.id}`}>
            <Card>
