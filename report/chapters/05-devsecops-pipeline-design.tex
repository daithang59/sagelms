...
Code quality/SAST & SonarCloud/SonarQube & Kiểm tra quality gate, bugs, vulnerabilities, code smells. \\
Secret scanning & Gitleaks & Phát hiện token, password, private key hoặc secret pattern trong repo. \\
Dependency/image/SBOM & Trivy & Quét phụ thuộc, image và tạo SBOM CycloneDX. \\
...
Signing & Cosign & Ký image digest và tạo attestation cho SBOM. \\
...
\begin{enumerate}
  ...
  \item Pipeline build Docker image, scan bằng Trivy, tạo SBOM, push lên Harbor, resolve digest và ký bằng Cosign.
...
\placeholderfigure{Pipeline DevSecOps tổng thể}{Thay placeholder này bằng sơ đồ Code -> PR Validation -> Build/Scan/SBOM/Sign -> Harbor -> Deploy PR -> FluxCD -> GKE -> Smoke Test.}
...
\begin{center}
\texttt{Build -> Scan -> Push -> Resolve digest -> SBOM/attestation -> Sign -> Deploy PR}
\end{center}

Việc ký và attest nên thực hiện trên digest đã resolve từ registry. Điều này giúp chữ ký gắn với nội dung image cụ thể, tránh rủi ro tag bị thay đổi sau khi ký.
...
Sau khi image được build, scan, push và ghi digest, pipeline mở deploy PR để cập nhật Kustomize overlay. Thay đổi manifest được review như code. Khi deploy PR được merge, FluxCD phát hiện commit mới, render manifest và reconcile vào namespace \texttt{sagelms-devsecops}.
